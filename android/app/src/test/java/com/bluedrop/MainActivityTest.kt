...
class MainActivityTest {
    ...
    fun `startBluetoothService should start foreground service on Android O and above`() {
        // When
        ReflectionHelpers.callInstanceMethod<Any>(activity, "startBluetoothService")
    ...
    @Test
    fun `startBluetoothService should publish selection through Essentials`() {
        // Device selection now flows through the Essentials state store, not
        // the start intent; verify the round trip the service actually reads.
        val testDevices = setOf("AA:BB:CC:DD:EE:FF", "11:22:33:44:55:66")
        Essentials.updateSelectedDevices(testDevices)
        assertEquals(testDevices, Essentials.selectedDevices.value)
    }
